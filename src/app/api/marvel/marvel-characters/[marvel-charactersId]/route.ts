...
export async function GET(request: NextRequest) {
  ...
  const characterId = request.nextUrl.pathname.split('/').pop();

  // Generate a timestamp and hash for the API request
  const ts = new Date().getTime().toString();
  const hash = crypto.createHash('md5').update(ts + privateKey + publicKey).digest('hex');

  const apiUrl = `https://gateway.marvel.com:443/v1/public/characters/${characterId}?ts=${ts}&apikey=${publicKey}&hash=${hash}`;
