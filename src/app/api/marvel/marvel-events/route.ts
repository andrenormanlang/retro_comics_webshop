...
export async function GET(request: NextRequest) {
  ...
  let apiUrl = `https://gateway.marvel.com:443/v1/public/events?ts=${ts}&apikey=${publicKey}&hash=${hash}&limit=${limit}`;

  if (searchTerm) {
    apiUrl += `&nameStartsWith=${encodeURIComponent(searchTerm)}`;
  } else {
