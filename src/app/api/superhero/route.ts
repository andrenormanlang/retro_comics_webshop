...
export async function GET(request: NextRequest) {
    ...
    if (!SUPERHERO_ACCESS_TOKEN) {
        return new NextResponse('Superhero API access token is not configured', { status: 500 });
    }
    ...
    if (searchTerm) {
        ...
        apiUrl = `https://superheroapi.com/api/${SUPERHERO_ACCESS_TOKEN}/search/${encodeURIComponent(searchTerm)}`;
    } else {
