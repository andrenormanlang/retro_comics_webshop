...
import { Publishers as PublisherType, SearchQuery } from "@/types/comic.types";

const ComicVinePublishersClient = () => {
  const pageSize = 15;
    ...
    <Container maxW="container.xl" centerContent p={4}>
        ...
        <Box>
          <Text fontSize="1.5em" mb={4} textAlign="center">
            {searchTerm
              ...
              : `You have a total of ${data.number_of_total_results} publishers in ${Math.ceil(
                  data.number_of_total_results / data.limit
                )} pages`}
          </Text>
        </Box>
      ...
      <SimpleGrid columns={{ base: 1, md: 3 }} spacing={10} width="100%">
        {validData?.map((publisher: PublisherType) => {
          ...
          return (
            <NextLink
              href={`/search/comic-vine/publishers/${publisher.id}?page=${currentPage}&query=${searchTerm}`}
              passHref
              key={publisher.id}
            >
              <motion.div
              ...
              >
                ...
                >
                  <Image
                    src={publisher.image.thumb_url}
                    alt={publisher.name || `Publisher ${publisher.id}`}
                    maxW="200px"
                    maxH="200px"
                    objectFit="contain"
                  />
...
export default ComicVinePublishersClient;
