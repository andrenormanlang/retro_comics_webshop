...
import { useGetMarvelEvents } from "@/hooks/marvel/useGetMarvelEvents";
import { MarvelEvent } from "@/types/marvel/marvel-comic.type";

const MarvelEventsClient: NextPage = () => {
	const pageSize = 15;
	...
	const { data, isLoading, isError, error } = useGetMarvelEvents(
		searchTerm,
		currentPage,
		pageSize
	);
			...
			<Container maxW="container.xl" centerContent p={4}>
				...
				>
					{data.data &&
						...
						data.data.results.map((marvelEvents: MarvelEvent) => (
							<NextLink
								href={`/search/marvel/marvel-events/${marvelEvents.id}?page=${currentPage}&query=${searchTerm}`}
								passHref
								key={marvelEvents.id}
							>
								<motion.div whileHover={{ scale: 1.05 }}>
									...
									>
										<Image
											src={`${marvelEvents.thumbnail.path}/portrait_uncanny.${marvelEvents.thumbnail.extension}`}
											alt={marvelEvents.title}
											maxW="300px"
											maxH="300px"
											objectFit="contain"
										/>
...
export default MarvelEventsClient;
